 ...
 *         by Prof. D. Probst
 * 
 *         $Revision: 1.5 $ $Last Revision Date: 2015/02/01 $
 */
public class BlockManager4 {
	...
	// Sets value of semaphore to -9. s1 will increase everytime it does phase1,
	// which will happens 10 times,
	// so the value becomes 1 in the end. This way phase2 can only begin when
	// all of phase1 is finish.
	...
	public static void main(String[] argv) {
		...
		System.out.println(soStack.getAccessCounter());
	} // main()
	...
	static class AcquireBlock extends BaseThread {
		...
		public void run() {
			...
			phase1();
			s1.V(); // increases value of s1
			s1.P(); // makes s1 wait until s1 value becomes 1,
					// which can only happen when all of phase1 are executed
			s1.V(); // increases s1's value to 1
					// so the other phase2 methods can execute
			mutex.P();
		...
		public void run() {
			...
			try {
				for (int i = 0; i < siThreadSteps; i++) {
					System.out.print("Stack Prober [TID=" + this.iTID
							+ "]: Stack state: ");
					...
					for (int s = 0; s < soStack.getISize(); s++)
						System.out
								.print((s == BlockManager4.soStack.getITop() ? "("
										: "[")
